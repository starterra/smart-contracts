...
use starterra_token::kyc_vault::{
    ConfigResponse, IsAcceptedResponse, IsAcceptedVerifiedResponse, IsVerifiedResponse,
};
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    Ok(ConfigResponse {
        ...
        kyc_provider_address: deps
            .api
            .addr_humanize(&state.kyc_provider_address)?
            .into_string(),
    })
}

pub fn query_verified(deps: Deps, address: String) -> StdResult<IsVerifiedResponse> {
...
pub fn query_accepted(deps: Deps, address: String) -> StdResult<IsAcceptedResponse> {
    let user_raw = deps.api.addr_canonicalize(&address)?;

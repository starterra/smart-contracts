...
use crate::querier::{check_if_user_participated_in_ido, check_if_user_stakes, load_balance};
use crate::state::{read_config, Config};
use cosmwasm_std::{CanonicalAddr, Coin, Deps, Env, MessageInfo, StdResult, Uint128};
...
pub fn user_stake_check(
    ...
    let staking_res: Result<Vec<bool>, ContractError> = staking_addresses
        .iter()
        .map(|contract_addr| {
            check_if_user_stakes(
                deps,
...
pub fn user_ido_check(
    ...
    let ido_res: Result<Vec<bool>, ContractError> = ido_addresses
        .iter()
        .map(|contract_addr| {
            check_if_user_participated_in_ido(
                deps,
                &deps.api.addr_humanize(contract_addr)?.into_string(),
                account_addr,
            )
...
pub fn get_ust_withdraw_coin(deps: Deps, env: Env) -> Result<Coin, ContractError> {
